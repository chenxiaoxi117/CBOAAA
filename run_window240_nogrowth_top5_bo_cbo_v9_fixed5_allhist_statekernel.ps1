...
$OUT = 'D:\CBOv2\results\window240_nogrowth_top5_static_bo_cbo_v9_fixed5_allhist_statekernel'
...
Write-Host '============================================================'
Write-Host 'Start BO+CBO top5 V9 | fixed5 + BO all-history + CBO state_gated_kernel + no TR'
Write-Host 'Purpose: compare against V8 recent80; this is intentionally NOT pure recent80.'
Write-Host "ROOT  = $ROOT"
Write-Host "OUT   = $OUT"
Write-Host "KEY   = $KEY"
Write-Host "Seeds = $($Seeds -join ',')"
Write-Host 'BO history  = all'
Write-Host 'CBO history = state_gated_kernel, topk=100, recent_keep=20, fallback=recent_context'
Write-Host '============================================================'
...
foreach ($seed in $Seeds) {
    foreach ($case in $cases) {
        ...
        $seedDir = "seed_$seed"
        $caseOut = Join-Path (Join-Path $OUT $seedDir) $name
        $stdout = Join-Path (Join-Path $OUT 'logs') "$seedDir.$name.stdout.log"
        $stderr = Join-Path (Join-Path $OUT 'logs') "$seedDir.$name.stderr.log"

        Write-Host ''
        Write-Host '============================================================'
        Write-Host "Running seed=$seed | $name"
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Host ''
            Write-Host "[FAILED] seed=$seed | $name"
            Write-Host "ExitCode = $LASTEXITCODE"
            Write-Host '===== stderr tail ====='
            Get-Content -LiteralPath $stderr -Tail 120
            Write-Host '===== stdout tail ====='
            Get-Content -LiteralPath $stdout -Tail 80
            throw "Case failed: seed=$seed | $name"
        }

        Write-Host "[OK] seed=$seed | $name"
    }
}

Write-Host ''
Write-Host '============================================================'
Write-Host 'All BO+CBO V9 fixed5 all-history/state-kernel tests finished.'
