...
# Three paired sigma ablations on the five difficult static scenes.
# Usage: MAX_JOBS=3 bash run_top5_v11_sigma_abc_s43.sh 43

ROOT="${ROOT:-/home/ecs-user/CBO}"
SEED="${1:-43}"
MAX_JOBS="${MAX_JOBS:-3}"
OUT="${OUT:-$ROOT/result/top5_v11_sigma_abc_s${SEED}}"
LOG_DIR="$OUT/logs"
...
run_case() {
  ...
  mkdir -p "$case_out"
  echo "[RUN ] $variant $scene calibration=$calibration acq=$acq_mode eta=$eta"
  ...
  python -m new_tr_split \
    ...
    --cbo-sigma-calibration "$calibration" \
    --cbo-sigma-calibration-buffer-size 50 \
    --cbo-sigma-calibration-min-samples 10 \
    --cbo-sigma-calibration-use-in-acq "$acq_mode" \
    --cbo-sigma-calibration-eta "$eta" \
...
calibrations=("off" "on" "on")
acq_modes=("false" "false" "soft")
etas=("0.25" "0.25" "0.25")

lams=("3.0" "3.0" "3.0" "2.6" "2.6")
probs=("10,40,50" "30,60,10" "10,30,60" "20,10,70" "70,20,10")
scenes=(
  "lambda3p0_rt10_batch40_ai50"
  "lambda3p0_rt30_batch60_ai10"
  "lambda3p0_rt10_batch30_ai60"
  "lambda2p6_rt20_batch10_ai70"
  "lambda2p6_rt70_batch20_ai10"
)

echo "============================================================"
echo "V11 sigma A/B/C top5 ablation"
echo "ROOT=$ROOT"
echo "OUT=$OUT"
echo "SEED=$SEED"
...
for vi in 0 1 2; do
  for si in 0 1 2 3 4; do
    run_case \
      "${variants[$vi]}" "${calibrations[$vi]}" "${acq_modes[$vi]}" "${etas[$vi]}" \
      "${lams[$si]}" "${probs[$si]}" "${scenes[$si]}" &
    running=$((running + 1))
...
if [[ "$summary_count" -ne 15 || "$config_count" -ne 15 || "$round_count" -ne 30 || -s "$FAIL_FILE" ]]; then
  exit 1
fi
