...
if __name__ == "__main__":
    ...
    parser.add_argument("--phase-reference-reuse-mode", choices=["significant_external", "exact_only"], default=getattr(CFG, "PHASE_REFERENCE_REUSE_MODE", "significant_external"), help="Reference cache reuse policy: legacy significant-scene grouping or exact/tolerance-equivalent external scenes only")
    ...
    parser.add_argument("--cbo-history-select-mode", choices=["recent", "recent_context", "recent_context_elite", "hybrid", "state_gated_kernel", "global_context_threshold", "external_internal_threshold", "external_internal_decay", "phase_adaptive_reuse", "phase_hierarchical_reuse"], default=getattr(CFG, "DEFAULT_CBO_HISTORY_SELECT_MODE", "recent"), help="CBO-only stability history selection mode")
    parser.add_argument("--cbo-history-select-targets", default="", help="Comma-separated CBO method keys that receive CLI history-selection overrides; empty applies to all CBO methods")
    ...
    parser.add_argument("--cbo-history-target-rows", type=int, default=getattr(CFG, "DEFAULT_CBO_HISTORY_TARGET_ROWS", 80), help="external_internal_decay: target GP training rows")
    parser.add_argument("--cbo-history-similar-keep", type=int, default=getattr(CFG, "DEFAULT_CBO_HISTORY_SIMILAR_KEEP", 16), help="external_internal_decay: similar-history rows retained after current-phase data is sufficient")
    parser.add_argument("--cbo-history-global-keep", type=int, default=getattr(CFG, "DEFAULT_CBO_HISTORY_GLOBAL_KEEP", 4), help="external_internal_decay: global rows always retained")
    parser.add_argument("--cbo-history-global-max", type=int, default=getattr(CFG, "DEFAULT_CBO_HISTORY_GLOBAL_MAX", 20), help="external_internal_decay: maximum global fallback rows when similar history is insufficient")
    parser.add_argument("--cbo-phase-memory-keep", type=int, default=getattr(CFG, "DEFAULT_CBO_PHASE_MEMORY_KEEP", 10), help="phase_adaptive_reuse: similar historical rows retained after 80 current-phase rows")
    parser.add_argument("--cbo-phase-component-sim-threshold", type=float, default=getattr(CFG, "DEFAULT_CBO_PHASE_COMPONENT_SIM_THRESHOLD", 0.70), help="phase_adaptive_reuse: intensity OR task-mix component similarity threshold")
    parser.add_argument("--cbo-phase-exact-intensity-tol", type=float, default=getattr(CFG, "DEFAULT_CBO_PHASE_EXACT_INTENSITY_TOL", 0.10), help="phase_adaptive_reuse: exact-scene arrival intensity tolerance")
    parser.add_argument("--cbo-phase-exact-taskmix-l1-tol", type=float, default=getattr(CFG, "DEFAULT_CBO_PHASE_EXACT_TASKMIX_L1_TOL", 0.06), help="phase_adaptive_reuse: exact-scene task-mix L1 tolerance")
    parser.add_argument("--cbo-phase-taskmix-min-sim", type=float, default=getattr(CFG, "DEFAULT_CBO_PHASE_TASKMIX_MIN_SIM", 0.70), help="phase_hierarchical_reuse: mandatory minimum task-mix similarity")
    parser.add_argument("--cbo-phase-overall-sim-threshold", type=float, default=getattr(CFG, "DEFAULT_CBO_PHASE_OVERALL_SIM_THRESHOLD", 0.65), help="phase_hierarchical_reuse: minimum task-mix-dominant overall similarity")
    parser.add_argument("--cbo-phase-taskmix-weight", type=float, default=getattr(CFG, "DEFAULT_CBO_PHASE_TASKMIX_WEIGHT", 0.75), help="phase_hierarchical_reuse: task-mix exponent in geometric overall similarity")
    parser.add_argument("--cbo-phase-similar-initial-cap", type=int, default=getattr(CFG, "DEFAULT_CBO_PHASE_SIMILAR_INITIAL_CAP", 40), help="phase_hierarchical_reuse: maximum non-exact transfer rows at phase start")
    parser.add_argument("--cbo-phase-similar-decay-end", type=int, default=getattr(CFG, "DEFAULT_CBO_PHASE_SIMILAR_DECAY_END", 80), help="phase_hierarchical_reuse: current-phase sample count where non-exact transfer reaches zero")
    parser.add_argument("--cbo-phase-archive-per-scene", type=int, default=getattr(CFG, "DEFAULT_CBO_PHASE_ARCHIVE_PER_SCENE", 80), help="phase_hierarchical_reuse: representative expired rows retained per stable external scene")
    parser.add_argument("--cbo-prediction-error-reexplore", type=_parse_bool_arg, default=getattr(CFG, "DEFAULT_CBO_PREDICTION_ERROR_REEXPLORE", False), help="Trigger adaptive exploration cooldown after a large posterior prediction error, without requiring TR")
    parser.add_argument("--cbo-external-intensity-max-diff", type=float, default=getattr(CFG, "DEFAULT_CBO_EXTERNAL_INTENSITY_MAX_DIFF", 0.5), help="Hard maximum arrival-intensity difference for external history reuse; negative disables")
    parser.add_argument("--cbo-external-lengthscale-intensity", type=float, default=getattr(CFG, "DEFAULT_CBO_EXTERNAL_LENGTHSCALE_INTENSITY", 0.5), help="External similarity kernel lengthscale for arrival intensity")
    parser.add_argument("--cbo-external-lengthscale-taskmix", type=float, default=getattr(CFG, "DEFAULT_CBO_EXTERNAL_LENGTHSCALE_TASKMIX", 0.35), help="External similarity kernel lengthscale for task-mix ratios")
    ...
    CFG.PHASE_REFERENCE_REUSE_MODE = str(args.phase_reference_reuse_mode)
    ...
    CFG.CBO_HISTORY_SELECT_TARGETS = str(args.cbo_history_select_targets or "")
    ...
    CFG.CBO_HISTORY_TARGET_ROWS = int(args.cbo_history_target_rows)
    CFG.CBO_HISTORY_SIMILAR_KEEP = int(args.cbo_history_similar_keep)
    CFG.CBO_HISTORY_GLOBAL_KEEP = int(args.cbo_history_global_keep)
    CFG.CBO_HISTORY_GLOBAL_MAX = int(args.cbo_history_global_max)
    CFG.CBO_PHASE_MEMORY_KEEP = int(args.cbo_phase_memory_keep)
    CFG.CBO_PHASE_COMPONENT_SIM_THRESHOLD = float(args.cbo_phase_component_sim_threshold)
    CFG.CBO_PHASE_EXACT_INTENSITY_TOL = float(args.cbo_phase_exact_intensity_tol)
    CFG.CBO_PHASE_EXACT_TASKMIX_L1_TOL = float(args.cbo_phase_exact_taskmix_l1_tol)
    CFG.CBO_PHASE_TASKMIX_MIN_SIM = float(args.cbo_phase_taskmix_min_sim)
    CFG.CBO_PHASE_OVERALL_SIM_THRESHOLD = float(args.cbo_phase_overall_sim_threshold)
    CFG.CBO_PHASE_TASKMIX_WEIGHT = float(np.clip(args.cbo_phase_taskmix_weight, 0.0, 1.0))
    CFG.CBO_PHASE_SIMILAR_INITIAL_CAP = int(args.cbo_phase_similar_initial_cap)
    CFG.CBO_PHASE_SIMILAR_DECAY_END = int(args.cbo_phase_similar_decay_end)
    CFG.CBO_PHASE_ARCHIVE_PER_SCENE = int(args.cbo_phase_archive_per_scene)
    CFG.CBO_PREDICTION_ERROR_REEXPLORE = bool(args.cbo_prediction_error_reexplore)
    CFG.CBO_EXTERNAL_INTENSITY_MAX_DIFF = float(args.cbo_external_intensity_max_diff)
    CFG.CBO_EXTERNAL_LENGTHSCALE_INTENSITY = float(args.cbo_external_lengthscale_intensity)
    CFG.CBO_EXTERNAL_LENGTHSCALE_TASKMIX = float(args.cbo_external_lengthscale_taskmix)
