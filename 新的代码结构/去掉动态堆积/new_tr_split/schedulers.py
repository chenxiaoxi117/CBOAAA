...
class BoltzmannScheduler:
    ...
    底层节点评分默认使用当前任务候选节点集合内的归一化：
    latency 先除以该任务 deadline budget，再在候选集内 min-max；
    energy 在候选集内 min-max；queue 使用当前槽位占用率；risk 使用
    当前任务的 deadline overrun ratio。
    """
    ...
    def __init__(self, np_rng=None, norm_mode="rolling"):
        ...
        if self.scheduler_score_norm_mode not in {"legacy", "candidate_minmax_deadline", "candidate_median", "candidate_iqr", "rolling_ema"}:
            raise ValueError(
                f"Unknown scheduler score norm mode={self.scheduler_score_norm_mode}. "
                "Expected legacy/candidate_minmax_deadline/candidate_median/candidate_iqr/rolling_ema."
            )
    ...
    def _init_task_type_norm_refs(self):
        ...
        for task_type in TASK_TYPE_ORDER:
            ...
            upload_size = get_task_upload_size(props)
            download_size = get_task_download_size(props)
            cpu_cycles = get_task_workload_cycles(props)

            energy_vals = []
            latency_vals = []
            source_nodes = [cfg["id"] for cfg in CFG.NODES_CFG if not _node_is_cloud(cfg)] or [0]
            for node_cfg in CFG.NODES_CFG:
                service_rate = get_effective_speed(node_cfg, task_type)
                t_comp = cpu_cycles / (service_rate + 1e-9)
                util_guess = min(1.0, float(get_task_required_cores(props)) / max(1.0, float(get_node_capacity_slots(node_cfg))))
                p_idle = get_node_idle_power(node_cfg)
                p_max = get_node_max_power(node_cfg)
                ...
                for origin_id in source_nodes:
                    trans_delays.append(
                        get_transmission_delay(origin_id, int(node_cfg["id"]), upload_size, include_local=True, direction="upload")
                        + get_transmission_delay(int(node_cfg["id"]), origin_id, download_size, include_local=True, direction="download")
                    )
                    trans_energies.append(
                        get_transmission_energy(origin_id, int(node_cfg["id"]), upload_size)
                        + get_transmission_energy(int(node_cfg["id"]), origin_id, download_size)
                    )
    ...
    def _node_score(self, task, node_idx, node, latency_w, energy_w, risk_w):
        ...
        return {
            ...
            "used_slots": float(max(0.0, float(getattr(node, "cpu_total", 1.0)) - float(getattr(node, "cpu_free", 0.0)))),
            "capacity_slots": float(max(1.0, float(getattr(node, "cpu_total", 1.0)))),
        }
    ...
    def _task_deadline_budget(self, task):
        create_t = float(getattr(task, "arrival_time", getattr(task, "create_time", self.current_time)))
        budget = float(getattr(task, "deadline", create_t)) - create_t
        if budget <= 0.0:
            budget = float(getattr(task, "duration_base", 1.0)) * max(float(getattr(task, "deadline_factor", 1.0)), 1.0)
        return max(budget, self.norm_floor)

    def _compute_candidate_deadline_minmax_norms(self, task, candidates):
        """Candidate-set normalization for a single task-node decision.

        Implements the paper-side form:
        D_ij/deadline_i -> candidate min-max, E_ij -> candidate min-max,
        Q_j -> used_slots/capacity_slots, R_ij -> clipped overrun ratio.
        """
        e_vals = np.array([c["energy_raw"] for c in candidates], dtype=float)
        response_vals = np.array([
            max(0.0, float(c.get("predicted_finish", self.current_time)) - float(getattr(task, "create_time", self.current_time)))
            for c in candidates
        ], dtype=float)
        deadline_budget = self._task_deadline_budget(task)
        delay_deadline_vals = response_vals / max(deadline_budget, self.norm_floor)
        ...
        def minmax(vals):
            ...
            return np.clip((vals - lo) / max(hi - lo, self.norm_floor), 0.0, self.norm_clip_max), lo, hi

        norm_l, d_min, d_max = minmax(delay_deadline_vals)
        norm_e, e_min, e_max = minmax(e_vals)
        if getattr(CFG, "USE_SCORE_RISK", True):
            norm_r = np.clip((response_vals - deadline_budget) / max(deadline_budget, self.norm_floor), 0.0, 1.0)
        else:
            norm_r = np.zeros(len(candidates), dtype=float)
        return norm_e, norm_l, norm_r, {
            "energy_ref": "candidate_minmax",
            "latency_ref": "candidate_deadline_minmax",
            "deadline_budget": float(deadline_budget),
            "deadline_delay_min": float(d_min),
            "deadline_delay_max": float(d_max),
            "energy_min": float(e_min),
            "energy_max": float(e_max),
            "energy_norm_mode": "candidate_minmax",
            "latency_norm_mode": "candidate_deadline_minmax",
            "risk_ref": "deadline_overrun_ratio" if getattr(CFG, "USE_SCORE_RISK", True) else "disabled",
        }
    ...
    def _compute_scheduler_norms(self, task, candidates):
        ...
        if mode == "legacy":
            ...
            norm_e, norm_l, norm_r, debug = self._compute_rolling_norms(task, candidates)
        elif mode == "candidate_minmax_deadline":
            norm_e, norm_l, norm_r, debug = self._compute_candidate_deadline_minmax_norms(task, candidates)
    ...
    def _score_candidate_components(self, c, latency_w, energy_w, risk_w, queue_w=0.0, alpha=None, tradeoff_mode="legacy"):
        ...
        cloud_penalty = (
            float(getattr(CFG, "CLOUD_NODE_PENALTY_WEIGHT", 0.0)) * float(bool(c.get("is_cloud", False)))
            if getattr(CFG, "USE_CLOUD_SCORE_PENALTY", True)
            else 0.0
        )
        mode = str(tradeoff_mode).lower()
        if mode == "legacy":
            latency_contribution = float(latency_w) * norm_l
            energy_contribution = float(energy_w) * norm_e
            risk_contribution = float(risk_w) * norm_r
            queue_contribution = float(queue_w) * norm_q
            service_contribution = latency_contribution + energy_contribution
            service_component = latency_contribution + risk_contribution + queue_contribution
            energy_component = energy_contribution
            score = service_contribution + risk_contribution + queue_contribution + cloud_penalty
            latency_energy_component = service_contribution
            ...
            c["risk_penalty"] = risk_contribution
            c["queue_penalty"] = queue_contribution
        else:
            ...
            latency_contribution = a * latency_component
            energy_contribution = (1.0 - a) * energy_component
            risk_contribution = float(risk_w) * norm_r
            queue_contribution = float(queue_w) * norm_q
            service_contribution = latency_contribution + energy_contribution
            risk_penalty = risk_contribution
            queue_penalty = queue_contribution
            latency_energy_component_unscaled = service_contribution
            ...
            score = latency_energy_component + risk_penalty + queue_penalty + cloud_penalty
        ...
        c["latency_contribution"] = float(latency_contribution)
        c["energy_contribution"] = float(energy_contribution)
        c["risk_contribution"] = float(risk_contribution)
        c["queue_contribution"] = float(queue_contribution)
        c["cloud_contribution"] = float(cloud_penalty)
        c["service_contribution"] = float(service_contribution)
        c["cloud_penalty"] = float(cloud_penalty)
    ...
    def select_node(self, task, nodes, theta_full):
        ...
        norm_q, queue_ref = self._compute_queue_slot_norms(candidates)
        norm_debug["queue_ref"] = queue_ref
        ...
        self.last_score_debug = {
            ...
            **norm_debug,
            ...
            "selected_latency_contribution": float(selected.get("latency_contribution", np.nan)),
            "selected_energy_contribution": float(selected.get("energy_contribution", np.nan)),
            "selected_service_contribution": float(selected.get("service_contribution", np.nan)),
            "selected_risk_contribution": float(selected.get("risk_contribution", np.nan)),
            "selected_queue_contribution": float(selected.get("queue_contribution", np.nan)),
            "selected_cloud_contribution": float(selected.get("cloud_contribution", np.nan)),
            "selected_risk_penalty": float(selected.get("risk_penalty", 0.0)),
            "selected_queue_penalty": float(selected.get("queue_penalty", 0.0)),
            "selected_cloud_penalty": float(selected.get("cloud_penalty", 0.0)),
...
class ConstrainedBoltzmannScheduler(BoltzmannScheduler):
    ...
    def _queue_pressure(self, task, node):
        effective_capacity = node._effective_capacity_for_task(task) if hasattr(node, "_effective_capacity_for_task") else node.cpu_total
        capacity_in_task_units = max(1.0, float(effective_capacity) / max(1.0, float(task.cpu_req)))
    ...
    def _node_score(self, task, node_idx, node, latency_w=None, energy_w=None, risk_w=None):
        ...
        effective_capacity = node._effective_capacity_for_task(task) if hasattr(node, "_effective_capacity_for_task") else node.cpu_total
        info["cpu_feasible"] = bool(task.cpu_req <= effective_capacity)
        return info
    ...
    def _apply_feasibility_filter(self, task, nodes, raw_infos, controls):
        debug = {
            ...
            "before_deadline_count": int(len(raw_infos)),
            "deadline_filter_reject_count": 0,
            "deadline_filter_reject_ratio": 0.0,
            "deadline_filter_used": False,
            "deadline_fallback_used": False,
            "fallback_risk_used": False,
            "fallback_reason": None,
            "cloud_pressure": None,
            "cloud_gate": float(controls.get("cloud_gate", getattr(CFG, "CLOUD_GATE_DEFAULT", 0.5))),
            "raw_cloud_candidate_count": int(sum(1 for c in raw_infos if c.get("is_cloud", False))),
            "raw_cloud_candidate_ratio": float(sum(1 for c in raw_infos if c.get("is_cloud", False))) / max(1, int(len(raw_infos))),
            "after_cloud_candidate_count": int(sum(1 for c in raw_infos if c.get("is_cloud", False))),
            "cloud_candidate_ratio": float(sum(1 for c in raw_infos if c.get("is_cloud", False))) / max(1, int(len(raw_infos))),
            "cloud_allowed_by_gate": None,
        }
        ...
        if getattr(CFG, "USE_CLOUD_GATE", True):
            ...
            debug["after_cloud_candidate_count"] = int(sum(1 for c in candidates if c.get("is_cloud", False)))
            debug["cloud_candidate_ratio"] = float(debug["after_cloud_candidate_count"]) / max(1, int(len(candidates)))
            debug["cloud_allowed_by_gate"] = bool(allow_cloud)
        ...
        if use_hard_deadline:
            ...
            debug["deadline_filter_used"] = True
            debug["before_deadline_count"] = int(len(before_deadline))
            feasible = [
            ...
            if feasible:
                ...
                debug["deadline_fallback_used"] = True
                debug["fallback_risk_used"] = True
            ...
            debug["deadline_filter_reject_count"] = max(0, int(len(before_deadline)) - int(len(candidates)))
            debug["deadline_filter_reject_ratio"] = float(debug["deadline_filter_reject_count"]) / max(1, int(len(before_deadline)))
        else:
            debug["after_deadline_count"] = int(len(candidates))
            debug["before_deadline_count"] = int(len(candidates))
    ...
    def select_node(self, task, nodes, theta_full):
        ...
        risk_w_raw = base_risk_w * float(controls.get("risk_scale", getattr(CFG, "RISK_SCALE_DEFAULT", 1.0)))
        queue_w_raw = float(controls.get("queue_w", getattr(CFG, "QUEUE_WEIGHT_DEFAULT", 1.0))) if getattr(CFG, "USE_QUEUE_PRESSURE_SCORE", True) else 0.0
        queue_base = float(getattr(CFG, "QUEUE_BASE_WEIGHTS", {"RT": 1.5, "Batch": 0.5, "AI": 1.0}).get(task.task_type, 1.0))
        queue_w = queue_base * queue_w_raw
        ...
        raw_infos = [self._node_score(task, idx, node, latency_w, energy_w, risk_w_raw) for idx, node in enumerate(nodes)]
        ...
        risk_mode = str(getattr(CFG, "SCHEDULER_RISK_SCORE_MODE", "fallback_tiebreak") or "fallback_tiebreak").lower()
        if risk_mode == "off":
            risk_score_multiplier = 0.0
        elif risk_mode == "legacy_main":
            risk_score_multiplier = 1.0
        else:
            risk_score_multiplier = (
                float(getattr(CFG, "SCHEDULER_RISK_FALLBACK_SCALE", 1.0))
                if bool(feasibility_debug.get("fallback_risk_used", False))
                else float(getattr(CFG, "SCHEDULER_RISK_TIEBREAKER_SCALE", 0.1))
            )
        risk_w = risk_w_raw * risk_score_multiplier
        ...
        self.last_score_debug = {
            ...
            "energy_w": float(energy_w),
            "risk_w": float(risk_w),
            "risk_w_raw": float(risk_w_raw),
            "w_risk_scale": float(controls.get("risk_scale", getattr(CFG, "RISK_SCALE_DEFAULT", 1.0))),
            "risk_score_mode": str(risk_mode),
            "risk_score_multiplier": float(risk_score_multiplier),
            "queue_w": float(queue_w),
            "queue_w_raw": float(queue_w_raw),
            "queue_base_task": float(queue_base),
            "cloud_gate": float(controls.get("cloud_gate", getattr(CFG, "CLOUD_GATE_DEFAULT", 0.5))),
            "alpha_rt": float(latency_weights.get("RT", np.nan)) if str(tradeoff_mode).lower() == "alpha_direct" else np.nan,
            "alpha_batch": float(latency_weights.get("Batch", np.nan)) if str(tradeoff_mode).lower() == "alpha_direct" else np.nan,
            "alpha_ai": float(latency_weights.get("AI", np.nan)) if str(tradeoff_mode).lower() == "alpha_direct" else np.nan,
            "scheduler_tradeoff_mode": tradeoff_mode,
            "scheduler_score_norm_mode": norm_debug.get("scheduler_score_norm_mode", getattr(CFG, "SCHEDULER_SCORE_NORM_MODE", "legacy")),
            "scheduler_alpha": float(alpha) if alpha is not None else None,
            "scheduler_alpha_source": alpha_source,
...
class DirectHeuristicScheduler:
    ...
    def _candidate_info(self, task, nodes, node_idx):
        ...
        effective_capacity = node._effective_capacity_for_task(task) if hasattr(node, "_effective_capacity_for_task") else getattr(node, "cpu_total", 1.0)
        capacity_in_task_units = max(1.0, float(effective_capacity) / max(1.0, float(getattr(task, "cpu_req", 1.0))))
        ...
        cpu_feasible = bool(float(getattr(task, "cpu_req", 1.0)) <= float(effective_capacity))
        capacity_slots = max(1.0, float(getattr(node, "cpu_total", 1.0)))
        used_slots = max(0.0, capacity_slots - float(getattr(node, "cpu_free", 0.0)))
    ...
    def _apply_candidate_norms(self, task, candidates):
        if not candidates or not getattr(CFG, "DIRECT_HEURISTIC_USE_CANDIDATE_NORM", True):
            return
        eps = max(float(getattr(CFG, "SCHEDULER_NORM_EPS", 1e-6)), 1e-12)
        e_vals = np.array([c["energy_raw"] for c in candidates], dtype=float)
        create_t = float(getattr(task, "arrival_time", getattr(task, "create_time", self.current_time)))
        budget = max(float(getattr(task, "deadline", create_t)) - create_t, eps)
        response_vals = np.array([max(0.0, float(c["predicted_finish"]) - create_t) for c in candidates], dtype=float)
        d_vals = response_vals / budget

        def minmax(vals):
            lo = float(np.nanmin(vals))
            hi = float(np.nanmax(vals))
            return np.clip((vals - lo) / max(hi - lo, eps), 0.0, float(getattr(CFG, "SCHEDULER_NORM_CLIP_MAX", 3.0)))

        norm_e = minmax(e_vals)
        norm_l = minmax(d_vals)
        norm_r = np.clip((response_vals - budget) / budget, 0.0, 1.0)
        norm_q = np.clip(
            np.array([float(c.get("used_slots", 0.0)) / max(float(c.get("capacity_slots", 1.0)), eps) for c in candidates], dtype=float),
            0.0,
            1.0,
        )
        for idx, c in enumerate(candidates):
            c["norm_e"] = float(norm_e[idx])
            c["norm_l"] = float(norm_l[idx])
            c["norm_risk"] = float(norm_r[idx])
            c["norm_queue"] = float(norm_q[idx])
            c["score_norm_mode"] = "candidate_minmax_deadline"

    def _score(self, c):
        mode = self.mode
        if getattr(CFG, "DIRECT_HEURISTIC_USE_CANDIDATE_NORM", True) and "norm_l" in c:
            cloud_penalty = (
                float(getattr(CFG, "CLOUD_NODE_PENALTY_WEIGHT", 0.0)) * float(bool(c.get("is_cloud", False)))
                if getattr(CFG, "USE_CLOUD_SCORE_PENALTY", True)
                else 0.0
            )
            base = float(c.get("norm_l", 0.0)) + float(c.get("norm_e", 0.0))
            risk = float(getattr(CFG, "DEADLINE_WEIGHT", 2.0)) * float(c.get("norm_risk", 0.0))
            queue = float(getattr(CFG, "QUEUE_WEIGHT_DEFAULT", 1.0)) * float(c.get("norm_queue", 0.0))
            if mode in {"greedy_direct_cost", "greedy_cost", "direct_greedy_cost", "queue_aware_greedy_direct", "queue_aware_greedy", "direct_queue_aware_greedy", "dpp_greedy"}:
                return base + risk + queue + cloud_penalty
    ...
    def select_node(self, task, nodes, theta_full=None):
        ...
        if self.mode in {"round_robin_direct", "round_robin", "rr", "roundrobin"}:
            ...
            self._apply_candidate_norms(task, candidates)
            for c in candidates:
        ...
        self.last_score_debug = {
            ...
            "selected_norm_e": float(selected.get("norm_e", np.nan)),
            "selected_norm_l": float(selected.get("norm_l", np.nan)),
            "selected_norm_risk": float(selected.get("norm_risk", np.nan)),
            "selected_norm_queue": float(selected.get("norm_queue", np.nan)),
            "scheduler_score_norm_mode": str(selected.get("score_norm_mode", "raw_legacy")),
