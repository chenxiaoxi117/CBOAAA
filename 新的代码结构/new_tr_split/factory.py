...
class ConnectedFactory:
    ...
    def reset(self, use_batch=False):
        ...
        self.perf_log = {
            ...
            "selected_queue_penalty_last": [], "selected_cloud_penalty_last": [], "selected_latency_energy_component_last": [],
            ...
            "selected_score_last": [],
            "latency_contribution_last": [], "energy_contribution_last": [], "service_contribution_last": [],
            "risk_contribution_last": [], "queue_contribution_last": [], "cloud_contribution_last": [],
            "deadline_filter_reject_ratio": [], "fallback_risk_used_ratio": [],
            "cloud_candidate_ratio": [], "cloud_selected_ratio": [],
            "w_queue": [], "w_queue_effective": [], "w_risk_scale": [], "cloud_gate": [],
            "W_RT_Latency_last": [], "W_Batch_Latency_last": [], "W_AI_Latency_last": [],
            "W_Queue_last": [], "W_Risk_Scale_last": [], "Cloud_Gate_last": [],
            "W_Energy_Scale_last": [], "W_RT_Energy_last": [], "W_Batch_Energy_last": [], "W_AI_Energy_last": [],
            "alpha_rt": [], "alpha_batch": [], "alpha_ai": [],
            "window_delay_deadline_norm": [], "window_censored_avg_delay": [], "window_energy_per_arrival": [],
            "window_energy_norm": [], "window_rt_violation_rate": [], "window_unfinished_rate": [],
            "window_backlog_growth": [], "window_backlog_growth_rate": [], "window_effective_avg_lateness": [],
            "window_cost_formula": []
        }
        self.window_history = []
    ...
    def _record_window_log(self, metrics, state, stable, theta, context_vec=None):
        ...
        for key in [
            "window_delay_deadline_norm", "window_censored_avg_delay", "window_energy_per_arrival",
            "window_energy_norm", "window_rt_violation_rate", "window_rt_violation_excess",
            "window_unfinished_arrivals", "window_unfinished_rate", "window_backlog_growth",
            "window_backlog_growth_rate", "window_effective_avg_lateness", "window_cost_formula",
            "window_cost_delay_weight", "window_cost_energy_weight", "window_cost_rt_violation_weight",
            "window_cost_unfinished_weight", "window_cost_backlog_growth_weight",
        ]:
            self.perf_log.setdefault(key, []).append(metrics.get(key))
        ...
        self.perf_log["completion_rate"].append(metrics.get("completion_rate", 0.0))
        self.perf_log.setdefault("completion_ratio", []).append(metrics.get("completion_ratio", 0.0))
        ...
        deployed_theta = list(getattr(self, "current_control_vector", theta) or [])
        feature_names = list(getattr(CFG, "FEATURE_NAMES", []))

        def theta_value(name):
            try:
                idx = feature_names.index(name)
                if idx < len(deployed_theta):
                    return float(deployed_theta[idx])
            except Exception:
                pass
            return None

        w_rt_latency = theta_value("W_RT_Latency")
        w_batch_latency = theta_value("W_Batch_Latency")
        w_ai_latency = theta_value("W_AI_Latency")
        w_queue_full = theta_value("W_Queue")
        w_risk_scale_full = theta_value("W_Risk_Scale")
        cloud_gate_full = theta_value("Cloud_Gate")
        w_rt_energy = theta_value("W_RT_Energy")
        w_batch_energy = theta_value("W_Batch_Energy")
        w_ai_energy = theta_value("W_AI_Energy")
        energy_vals = [w_rt_energy, w_batch_energy, w_ai_energy]
        if all(v is not None for v in energy_vals) and max(energy_vals) - min(energy_vals) <= 1e-9:
            w_energy_scale = float(w_rt_energy)
        else:
            w_energy_scale = None
        self.perf_log.setdefault("W_RT_Latency_last", []).append(w_rt_latency)
        self.perf_log.setdefault("W_Batch_Latency_last", []).append(w_batch_latency)
        self.perf_log.setdefault("W_AI_Latency_last", []).append(w_ai_latency)
        self.perf_log.setdefault("W_Queue_last", []).append(w_queue_full)
        self.perf_log.setdefault("W_Risk_Scale_last", []).append(w_risk_scale_full)
        self.perf_log.setdefault("Cloud_Gate_last", []).append(cloud_gate_full)
        self.perf_log.setdefault("W_Energy_Scale_last", []).append(w_energy_scale)
        self.perf_log.setdefault("W_RT_Energy_last", []).append(w_rt_energy)
        self.perf_log.setdefault("W_Batch_Energy_last", []).append(w_batch_energy)
        self.perf_log.setdefault("W_AI_Energy_last", []).append(w_ai_energy)
        ...
        self.perf_log.setdefault("alpha_rt", []).append(sched_debug.get("alpha_rt"))
        self.perf_log.setdefault("alpha_batch", []).append(sched_debug.get("alpha_batch"))
        self.perf_log.setdefault("alpha_ai", []).append(sched_debug.get("alpha_ai"))
        self.perf_log.setdefault("w_queue", []).append(sched_debug.get("w_queue", sched_debug.get("queue_w_raw")))
        self.perf_log.setdefault("w_queue_effective", []).append(sched_debug.get("w_queue_effective", sched_debug.get("queue_w")))
        self.perf_log.setdefault("w_risk_scale", []).append(sched_debug.get("w_risk_scale"))
        self.perf_log.setdefault("cloud_gate", []).append(sched_debug.get("cloud_gate"))
        self.perf_log.setdefault("selected_latency_component_last", []).append(sched_debug.get("selected_latency_component"))
        self.perf_log.setdefault("latency_contribution_last", []).append(sched_debug.get("selected_latency_contribution"))
        self.perf_log.setdefault("energy_contribution_last", []).append(sched_debug.get("selected_energy_contribution"))
        self.perf_log.setdefault("service_contribution_last", []).append(sched_debug.get("selected_service_contribution"))
        self.perf_log.setdefault("risk_contribution_last", []).append(sched_debug.get("selected_risk_contribution"))
        self.perf_log.setdefault("queue_contribution_last", []).append(sched_debug.get("selected_queue_contribution"))
        self.perf_log.setdefault("cloud_contribution_last", []).append(sched_debug.get("selected_cloud_contribution"))
        ...
        self.perf_log.setdefault("selected_cloud_penalty_last", []).append(sched_debug.get("selected_cloud_penalty"))
        ...
        self.perf_log.setdefault("deadline_filter_reject_ratio", []).append(sched_debug.get("deadline_filter_reject_ratio"))
        self.perf_log.setdefault("fallback_risk_used_ratio", []).append(sched_debug.get("fallback_risk_used_ratio"))
        self.perf_log.setdefault("cloud_candidate_ratio", []).append(sched_debug.get("cloud_candidate_ratio"))
        self.perf_log.setdefault("cloud_selected_ratio", []).append(sched_debug.get("cloud_selected_ratio"))
    ...
    def _system_backlog_count(self):
        ...
        in_transit = sum(1 for ev in self.events if ev.type in (EventType.TRANS_FINISH, EventType.DOWNLOAD_FINISH))
    ...
    def run_continuous(self, theta=None, external_samples=None, model_prior=None, window_end=None, eval_state=None, eval_context=None, feedback_control=None):
        ...
        while True:
            ...
            if ev.type == EventType.TASK_ARRIVAL:
                ...
                snapshot.arrived_tasks[task.id] = task
                ...
                trans_delay = get_transmission_delay(origin, node_idx, getattr(task, "upload_size", task.data_size), include_local=True, direction="upload")
                ...
                task.transmission_energy = get_transmission_energy(origin, node_idx, getattr(task, "upload_size", task.data_size))
                ...
                node.release(task)
                self._try_start(node)
                task.compute_finish_time = float(self.current_time)
                origin = task.origin_node_id if task.origin_node_id >= 0 else task.arrival_node_idx
                download_delay = get_transmission_delay(task.arrival_node_idx, origin, getattr(task, "download_size", 0.0), include_local=True, direction="download")
                if getattr(CFG, "USE_LINK_QUEUE", False):
                    link_key = (int(task.arrival_node_idx), int(origin))
                    download_start = max(self.current_time, float(self.link_busy_until.get(link_key, self.current_time)))
                    finish_time = download_start + download_delay
                    self.link_busy_until[link_key] = finish_time
                else:
                    finish_time = self.current_time + download_delay
                download_energy = get_transmission_energy(task.arrival_node_idx, origin, getattr(task, "download_size", 0.0))
                task.transmission_energy += download_energy
                snapshot.transmission_energy += download_energy
                snapshot.total_energy += download_energy
                snapshot.total_energy_real += download_energy
                self.cumulative_energy += download_energy
                self.cumulative_energy_real += download_energy
                task.finish_time = float(finish_time)
                heapq.heappush(self.events, Event(finish_time, EventType.DOWNLOAD_FINISH, task))
            elif ev.type == EventType.DOWNLOAD_FINISH:
                task = ev.payload
                # Compute energy is interval-integrated; per-task direct energy is transmission only.
        ...
        prev_backlog_count = getattr(self, "_prev_window_backlog_count", None)
        metrics = snapshot.to_metrics(
            self.cumulative_energy,
            backlog=backlog_count,
            cumulative_energy_real=self.cumulative_energy_real,
            prev_backlog=prev_backlog_count,
        )
        self._prev_window_backlog_count = int(backlog_count)
        ...
        if window_scheduler_debugs:
            ...
            def bool_ratio_debug(key):
                vals = []
                for d in window_scheduler_debugs:
                    if key in d:
                        vals.append(1.0 if bool(d.get(key)) else 0.0)
                return float(np.mean(vals)) if vals else None
            for src_key, dst_key in [
                ("deadline_filter_reject_ratio", "deadline_filter_reject_ratio"),
                ("cloud_candidate_ratio", "cloud_candidate_ratio"),
                ("queue_w_raw", "w_queue"),
                ("queue_w", "w_queue_effective"),
                ("w_risk_scale", "w_risk_scale"),
                ("cloud_gate", "cloud_gate"),
                ("alpha_rt", "alpha_rt"),
                ("alpha_batch", "alpha_batch"),
                ("alpha_ai", "alpha_ai"),
            ]:
                last_sched_debug[dst_key] = mean_debug(src_key)
            last_sched_debug["fallback_risk_used_ratio"] = bool_ratio_debug("fallback_risk_used")
            last_sched_debug["cloud_selected_ratio"] = bool_ratio_debug("selected_is_cloud")
        else:
            last_sched_debug = {}
    ...
    def _try_start(self, node):
        ...
        for t in list(node.ready_queue):
            if node.allocate(t):
                ...
                service_rate = node.effective_speed(t)
                t.finish_time = self.current_time + t.cpu_cycles / (service_rate + 1e-9)
