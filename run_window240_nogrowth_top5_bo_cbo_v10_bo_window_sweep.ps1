...
Write-Host '============================================================'
Write-Host 'Start BO+CBO top5 rerun | v10 BO recent-window sweep'
Write-Host 'CBO recent window is not overridden in this script.'
Write-Host "ROOT      = $ROOT"
Write-Host "OUT       = $OUT"
Write-Host "KEY       = $KEY"
Write-Host "Seed      = $Seed"
Write-Host "BoWindows = $($BoWindows -join ',')"
Write-Host '============================================================'

foreach ($boWindow in $BoWindows) {
    foreach ($case in $cases) {
        $name = $case.name
        $lam = $case.lam
        $probs = $case.probs

        $runDir = "bo_window_$boWindow"
        $caseOut = Join-Path (Join-Path $OUT $runDir) $name
        $stdout = Join-Path (Join-Path $OUT 'logs') "bo_window_$boWindow.$name.stdout.log"
        $stderr = Join-Path (Join-Path $OUT 'logs') "bo_window_$boWindow.$name.stderr.log"

        Write-Host ''
        Write-Host '============================================================'
        Write-Host "Running bo_window=$boWindow | $name"
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Host ''
            Write-Host "[FAILED] bo_window=$boWindow | $name"
            Write-Host "ExitCode = $LASTEXITCODE"
            Write-Host '===== stderr tail ====='
            Get-Content -LiteralPath $stderr -Tail 120
            Write-Host '===== stdout tail ====='
            Get-Content -LiteralPath $stdout -Tail 80
            throw "Case failed: bo_window=$boWindow | $name"
        }

        Write-Host "[OK] bo_window=$boWindow | $name"
    }
}

Write-Host ''
Write-Host '============================================================'
Write-Host 'All BO+CBO v10 BO-window sweep tests finished.'
