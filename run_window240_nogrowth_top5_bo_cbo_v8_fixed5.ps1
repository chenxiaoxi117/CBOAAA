...
if ($TrustRegion) {
    $OUT = 'D:\CBOv2\results\window240_nogrowth_top5_static_bo_cbo_v8_fixed5_tr'
    $RunLabel = 'window240 + nogrowth v8 + fixed5 shared probes + CBO-first reference + adaptive TR'
} else {
    $OUT = 'D:\CBOv2\results\window240_nogrowth_top5_static_bo_cbo_v8_fixed5'
    $RunLabel = 'window240 + nogrowth v8 + fixed5 shared probes + CBO-first reference'
}
...
Write-Host '============================================================'
Write-Host "Start BO+CBO top5 rerun | $RunLabel"
Write-Host "ROOT = $ROOT"
Write-Host "OUT  = $OUT"
Write-Host "KEY  = $KEY"
Write-Host "TR   = $TrustRegion"
Write-Host '============================================================'
...
foreach ($case in $cases) {
    ...
    if ($TrustRegion) {
        $args += @(
            '--cbo-tr-mode', 'adaptive',
            '--cbo-tr-radius-init', '0.10',
            '--cbo-tr-radius-min', '0.04',
            '--cbo-tr-radius-max', '0.35',
            '--cbo-tr-update-mode', 'best_so_far',
            '--cbo-tr-anchor-mode', 'posterior_mean'
        )
    }
...
Write-Host ''
Write-Host '============================================================'
Write-Host 'All BO+CBO v8 fixed5 tests finished.'
